...
const char * const BITCOIN_CONF_FILENAME = "easynode.conf";
const char * const BITCOIN_PID_FILENAME = "easynoded.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "easynode" is a composite category enabling all EasyNode-related debug output
            if(ptrCategory->count(string("easynode"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EasyNodeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EasyNodeCore
    // Mac: ~/Library/Application Support/EasyNodeCore
    // Unix: ~/.easynodecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EasyNodeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EasyNodeCore";
#else
    // Unix
    return pathRet / ".easynodecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty easynode.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override easynode.conf
