 ...
 *
 *   // network
 *   CEasyNodeNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.easynode.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CEasyNodeNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CEasyNodeNetwork(UniValue objIn)
    {
...
// // can be under: EasyNodeNetwork
// //   -- signature requirements : Key1(User)
// class CEasyNodeNetworkVariable : public CGovernanceObject
// {
// private:
