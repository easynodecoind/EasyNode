...
*Setup instructions for a Gitian build of EasyNode Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the EasyNode
...
- [Building EasyNode Core](#building-easynode-core)
...
Clone the git repositories for EasyNode Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/easynodecoind/easynode
```
...
in the EasyNode Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building EasyNode Core
----------------

To build EasyNode Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the EasyNode Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/easynode/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/easynodecoind/easynode
...
For example:
```bash
URL=https://github.com/crowning-/easynode.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit easynode=${COMMIT} --url easynode=${URL} ../easynode/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit easynode=${COMMIT} --url easynode=${URL} ../easynode/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit easynode=${COMMIT} --url easynode=${URL} ../easynode/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the easynode git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../easynode/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/easynode.git
SIGPATH=/some/root/path/easynode-detached-sigs.git

./bin/gbuild --url easynode=${BTCPATH},signature=${SIGPATH} ../easynode/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/easynode-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/easynode-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/easynode-osx-build.assert
```
...
[easynode/gitian.sigs](https://github.com/easynodecoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
